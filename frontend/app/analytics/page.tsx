...
export default function AnalyticsPage() {
  ...
  const intentData = ['Need urgently', 'Ready to start soon', 'Comparing options', 'Want to learn more', 'Just exploring'].map(intent => ({
  ...
  const budgetIntentMatrix = ['High', 'Medium', 'Low'].map(budget => ({
    budget,
    'Need urgently': leads.filter(l => l.budget === budget && l.intent === 'Need urgently').length,
    ...
    'Want to learn more': leads.filter(l => l.budget === budget && l.intent === 'Want to learn more').length,
    'Just exploring': leads.filter(l => l.budget === budget && l.intent === 'Just exploring').length,
  }));

  const sourceData = leads.reduce((acc: Record<string, number>, l) => {
    const s = l.source || 'Unknown'; acc[s] = (acc[s] || 0) + 1; return acc;
  }, {});
  const sourcePieData = Object.entries(sourceData).map(([name, value], i) => ({ name, value, color: ['#e11d48', '#f59e0b', '#22c55e', '#3b82f6'][i % 4] }));

  const interestScoreData = ['AI', 'Data Science', 'Machine Learning'].map(interest => ({
    interest,
  ...
  return (
    <div>
      ...
      <div className="kpi-grid" style={{ marginBottom: 24 }}>
        {[
          { label: 'Total Leads', value: analytics?.total_leads ?? 0, color: '#e11d48' },
            ...
            <div style={{ height: 260 }}>
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={200}>
                <BarChart data={budgetIntentMatrix} barCategoryGap="25%">
                  ...
                  <Bar dataKey="Need urgently" stackId="a" fill="#ef4444" />
                  <Bar dataKey="Ready to start soon" stackId="a" fill="#f59e0b" />
                  <Bar dataKey="Comparing options" stackId="a" fill="#e11d48" />
                  <Bar dataKey="Want to learn more" stackId="a" fill="#10b981" />
                  <Bar dataKey="Just exploring" stackId="a" fill="#3b82f6" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div style={{ height: 260 }}>
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={200}>
                <BarChart data={followupDistData} barCategoryGap="30%">
                  ...
                  <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(225,29,72,0.05)' }} />
                  <Bar dataKey="count" name="Leads" radius={[6, 6, 0, 0]}>
                    {followupDistData.map((entry, index) => (
                      <Cell key={index} fill={CHART_COLORS[(index + 5) % CHART_COLORS.length]} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
